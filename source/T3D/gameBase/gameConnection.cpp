...
Point3F gMouseWorldPt;							// 鼠标世界坐标
Point3F gMouseScreenPt;							// 鼠标屏幕坐标

Point3F gMouseWorldPt1;						// 鼠标世界坐标
Point3F gMouseScreenPt1;						// 鼠标屏幕坐标

Point3F gMouserayOri;   
Point3F gMouserayDir;                                  //瞄准向量

Point3F gGunOri_0;                                     //枪瞄准向量
Point3F gGunDir_0;
Point3F gGunDir_1;
Point3F gGunOri_1;
Point3F gGunDirAy[2];
...
Point3F gPcvrPointW0;							// 外设用枪1
Point3F gPcvrPointWAy[2];
Point3F gPcvrPointS0;							// 外设用枪1
Point3F gPcvrPointSAy[2];						// 外设用枪3-x

Point3F gPcvrPointW1;							// 外设用枪2
Point3F gPcvrPointS1;							// 外设用枪2

Point3F gPcvrPointW01;							// 外设用枪1
Point3F gPcvrPointS01;							// 外设用枪1

Point3F gPcvrPointW11;							// 外设用枪2
Point3F gPcvrPointS11;							// 外设用枪2

Point2I pt_currentPot0;
Point2I pt_currentPot1;

VectorF gGunV0;									// 第一把枪的朝向向量
VectorF gGunV1;									// 第二把枪的朝向向量
VectorF gGunVAy[2];

VectorF gGunP0;									// 第一把枪的位置
VectorF gGunP1;									// 第二把枪的位置

bool b_gPaused = false;
//pcvr* gPcvr = NULL;								// 硬件控制全局指针

//bool b_gPaused = false;
//bool b_gOnserverCreate = false;
//GuiBitmapCtrl* gLoadingGif = NULL;				//加载界面动画切换全局指针
//U32  m_gSaveTime = 0;

RayInfo MouseRayinfo;
/////////////////////////////////////////////////
RayInfo GunRayinfo_0;
RayInfo GunRayinfo_1;
RayInfo GunRayinfoAy[2];
/////////////////////////////////////////////////

// 这是为双枪游戏加的
//ShapeBase *GameConnection::g_pLeftPlayer = NULL;
//ShapeBase *GameConnection::g_pRightPlayer = NULL;

bool bGameTestSelf = false;
//采集器最小冷却时间=1000/60=16ms.(60->采集器的刷新率)
float TimeCameraMinFree = 16 * 2;
float TimeCameraMin = TimeCameraMinFree + 16;
...
DefineEngineFunction( setGameTestSelf, void, (bool flag),, "")
{
	bGameTestSelf = flag;
}

DefineEngineFunction( setGameTimeCameraMinFree, void, (float val),, "")
{
	TimeCameraMinFree = val;
}

DefineEngineFunction( setGameTimeCameraMin, void, (float val),, "")
{
	TimeCameraMin = TimeCameraMinFree + val;
}
