...
//
static U32 sCollisionWeiyiMask = TerrainObjectType       |
							InteriorObjectType      |
							WaterObjectType         | 
							VehicleObjectType       |
...
IMPLEMENT_CALLBACK( PlayerData, animationDone, void, ( Player* obj, const char* actionName ), ( obj, actionName ),
...
IMPLEMENT_CALLBACK( PlayerData, jumpDown, void, ( Player* obj ), ( obj ),
				   "@brief Called on the server when a scripted animation completes.\n\n"
				   "@param obj The Player object\n"
				   "@see Player::setActionThread() for setting a scripted animation and its 'hold' parameter to "
				   "determine if this callback is used.\n" );

IMPLEMENT_CALLBACK( PlayerData, jumpInair, void, ( Player* obj ), ( obj ),
				   "@brief Called on the server when a scripted animation completes.\n\n"
				   "@param obj The Player object\n"
				   "@see Player::setActionThread() for setting a scripted animation and its 'hold' parameter to "
				   "determine if this callback is used.\n" );
...
IMPLEMENT_CALLBACK( PlayerData, onLeaveMissionArea, void, ( Player* obj ), ( obj ),
   ...
   "@param obj The Player object\n"
   "@see MissionArea\n" );
IMPLEMENT_CALLBACK( PlayerData, onwriteActionInfo, void, ( Player* obj, const char* name, const char* actionName, Point3F p, Point3F box, U32 count ),(obj, name, actionName, p, box, count),"writeActionInfo");

IMPLEMENT_CALLBACK( PlayerData, OnAnimationTrigger, void, ( Player* obj, const char* actionName ), ( obj, actionName ),
   "@brief Called on the server when a scripted animation arrive at the trigger point .\n");

IMPLEMENT_CALLBACK( PlayerData, onDisabled, void, ( Player* obj, const char* state ), ( obj, state ),
				   "@brief Called on the server when npc damage arrive at the dead level.\n");
...
PlayerData::PlayerData()
{
   ...
   // damage locations
   boxHeadLeftPercentage  = -0.1f;
   boxHeadRightPercentage = 0.1f;
   /*boxHeadBackPercentage  = 0;
   boxHeadFrontPercentage = 1;*/
   boxTorsoLeftPercentage = -0.15f;
   boxTorsoRightPercentage = 0.15f;

   boxLegsLeftPercentage = -0.15f;
   boxLegsRightPercentage = 0.15f;
...
bool PlayerData::preload(bool server, String &errorStr)
{
   ...
   if ( mShape )
   {
	  ...
	  if( !DamageExplosion && DamageExplosionID != 0 )
	  {
		  if( Sim::findObject( DamageExplosionID, DamageExplosion ) == false)
		  {
			  Con::errorf( ConsoleLogEntry::General, "PlayerData::preload: Invalid packet, bad datablockId(DamageExplosion): 0x%x", DamageExplosionID );
		  }
		  AssertFatal(!(DamageExplosion && ((DamageExplosionID < DataBlockObjectIdFirst) || (DamageExplosionID > DataBlockObjectIdLast))),
			  "ShapeBaseData::preload: invalid DamageExplosion data");
	  }
   ...
   for ( int i = 0; i < ACTIONNUM; i++ )
   {
	   if (!actionArray[i] && actionArrayNum[i] != 0 )
		   if (!Sim::findObject(actionArrayNum[i],actionArray[i]))
			   Con::errorf(ConsoleLogEntry::General, "PlayerData::preload Invalid packet, bad datablockId(y_climb): 0x%x", actionArray[i]);
   }
   ...
   if ( mShape )
   {
	   mountBoxNode[0] = mShape->findNode("touBox");
	   mountBoxNode[1] = mShape->findNode("zuodabangziBox");
	   mountBoxNode[2] = mShape->findNode("zuoxiaobangziBox");
	   mountBoxNode[3] = mShape->findNode("zuodatuiBox");
	   mountBoxNode[4] = mShape->findNode("zuoxiaotuiBox");
	   mountBoxNode[5] = mShape->findNode("shentiBox");
	   mountBoxNode[6] = mShape->findNode("youdabangziBox");
	   mountBoxNode[7] = mShape->findNode("youxiaobangziBox");
	   mountBoxNode[8] = mShape->findNode("youdatuiBox");
	   mountBoxNode[9] = mShape->findNode("youxiaotuiBox");
	   mountBoxNode[10] = mShape->findNode("zhangyang0");
	   mountBoxNode[11] = mShape->findNode("zhangyang1");
	   mountBoxNode[12] = mShape->findNode("zhangyang2");
	   mountBoxNode[13] = mShape->findNode("zhangyang3");
	   mountBoxNode[14] = mShape->findNode("zhangyang4");
	   mountBoxNode[15] = mShape->findNode("zhangyang5");
	   mountBoxNode[16] = mShape->findNode("zhangyang6");
	   mountBoxNode[17] = mShape->findNode("zhangyang7");
	   mountBoxNode[18] = mShape->findNode("zhangyang8");
	   mountBoxNode[19] = mShape->findNode("zhangyang9");
	   mountBoxNode[20] = mShape->findNode("zhangyang10");
	   mountBoxNode[21] = mShape->findNode("zhangyang11");
	   mountBoxNode[22] = mShape->findNode("zhangyang12");
	   mountBoxNode[23] = mShape->findNode("zhangyang13");
   }
...
void PlayerData::getGroundInfo(TSShapeInstance* si, TSThread* thread,ActionAnimation *dp)
{
   ...
   if( dStricmp(dp->name, "run") == 0 )
   {
	   String str;
	   str = str.ToString(dp->speed);
	   //Con::executef( "buju", getName(), str );
   }
}
...
void PlayerData::initPersistFields()
{
   ...
   addGroup( "Collision" );
      ...
      addField("DamageExplosion", TYPEID< ExplosionData >() , Offset(DamageExplosion, PlayerData) );
      ...
      addField( "boxTorsoRightPercentage", TypeF32, Offset(boxTorsoRightPercentage, PlayerData),
         ...
         "Used when computing the damage location.\n"
         "@see Player::getDamageLocation" );
	  addField( "boxLegsLeftPercentage", TypeF32, Offset(boxLegsLeftPercentage, PlayerData),
		  "@brief Percentage of the player's bounding box depth that represents the back side of the head.\n\n"
		  "Used when computing the damage location.\n"
		  "@see Player::getDamageLocation" );
	  addField( "boxLegsRightPercentage", TypeF32, Offset(boxLegsRightPercentage, PlayerData),
		  "@brief Percentage of the player's bounding box depth that represents the front side of the head.\n\n"
		  "Used when computing the damage location.\n"
		  "@see Player::getDamageLocation" );
   ...
   addGroup( "Third Person" );
      ...
      addField( "allowImageStateAnimation", TypeBool, Offset(allowImageStateAnimation, PlayerData),
         ...
         "need to.\n");
	  addField( "isFly", TypeBool, Offset(isFly, PlayerData),
		  "@brief Allow mounted images to request a sequence be played on the Player.\n\n"
		  "When true a new thread is added to the player to allow for "
		  "mounted images to request a sequence be played on the player "
		  "through the image's state machine.  It is only optional so "
  ...
  addGroup( "action Data" );
   ...
   addField( "y_climb", TYPEID< DecalData >(), Offset(actionArray[0], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_SawHurt", TYPEID< DecalData >(), Offset(actionArray[1], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_death1", TYPEID< DecalData >(), Offset(actionArray[2], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_death2", TYPEID< DecalData >(), Offset(actionArray[3], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_death3", TYPEID< DecalData >(), Offset(actionArray[4], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_death4", TYPEID< DecalData >(), Offset(actionArray[5], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_death5", TYPEID< DecalData >(), Offset(actionArray[6], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_death6", TYPEID< DecalData >(), Offset(actionArray[7], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_death7", TYPEID< DecalData >(), Offset(actionArray[8], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_deathshoot", TYPEID< DecalData >(), Offset(actionArray[9], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_run3", TYPEID< DecalData >(), Offset(actionArray[10], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_fire1", TYPEID< DecalData >(), Offset(actionArray[11], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_fire2", TYPEID< DecalData >(), Offset(actionArray[12], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_fire3", TYPEID< DecalData >(), Offset(actionArray[13], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_fire4", TYPEID< DecalData >(), Offset(actionArray[14], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_headrootHurt1", TYPEID< DecalData >(), Offset(actionArray[15], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_headrootHurt2", TYPEID< DecalData >(), Offset(actionArray[16], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_Kicked", TYPEID< DecalData >(), Offset(actionArray[17], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_houtui", TYPEID< DecalData >(), Offset(actionArray[18], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_FalchionHurt", TYPEID< DecalData >(), Offset(actionArray[19], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_run2", TYPEID< DecalData >(), Offset(actionArray[20], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_hurt1", TYPEID< DecalData >(), Offset(actionArray[21], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_leftJumpWall", TYPEID< DecalData >(), Offset(actionArray[22], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
   addField( "y_rightJumpWall", TYPEID< DecalData >(), Offset(actionArray[23], PlayerData),
	   "@brief Decal to place on the ground for player footsteps.\n\n" );
  endGroup( "action Data" );
...
void PlayerData::packData(BitStream* stream)
{
   ...
   for (int i = 0; i < ACTIONNUM; i++ )
   {
	   if( stream->writeFlag( actionArray[i] ) )
	   {
		   stream->writeRangedU32( actionArray[i]->getId(), DataBlockObjectIdFirst,  DataBlockObjectIdLast );
	   }
   }
   ...
   stream->writeFlag(isFly);

   //受伤粒子相关
   if( stream->writeFlag( DamageExplosion != NULL ) )
   {
	   stream->writeRangedU32( DamageExplosion->getId(), DataBlockObjectIdFirst,  DataBlockObjectIdLast );
   }
}
...
void PlayerData::unpackData(BitStream* stream)
{
   ...
   for ( int i = 0; i < ACTIONNUM; i++ )
   {
	   if( stream->readFlag() )
	   {
		   actionArrayNum[i] = (S32) stream->readRangedU32(DataBlockObjectIdFirst, DataBlockObjectIdLast);
	   }
   }
   ...
   isFly = stream->readFlag();

   //受伤粒子相关
   if( stream->readFlag() )
   {
	   DamageExplosionID = stream->readRangedU32( DataBlockObjectIdFirst, DataBlockObjectIdLast );
   }
}
...
F32 Player::mGravity = -9.81f;     //重力加速度
...
Player::Player()
{
   ...
   for( U32 i = 0; i < 6; i++ )
   {
		mBrokenArray[i] = false;
   }

   for ( int i = 0; i < NumMountBox; i++ )
   {
	   mountBox[i] = -1;
   }
   bIsMountBox = false;
   ...
   bIsMoveState = false;
   mNewActionAnimation.init();
   isJump = false;
   bStopCurAction = false;
   bReversePlay = false;
   mWeiyiGravity = -50.0;

   m_DataType = PLAYER_DATA_EXPLODE;
   mLifeTime = 0;
}
...
void Player::processTick(const Move* move)
{
   ...
   if (delta.warpTicks > 0) {
      ...
      if ( isServerObject() || didRenderLastRender() || getControllingClient() )
      {
		 ...
		 //if( mVelocity.len() )
			//Con::printf("mVelocity.x = %f   mVelocity.y =  %f  mVelocity.z = %f", mVelocity.x, mVelocity.y, mVelocity.z );
      }
   ...
   if( mLifeTime >= TickMs )
   {
	   ...
	   if( mLifeTime < TickMs )
	   {
		   mLifeTime = 0;
		   const char* script = 0;
		   const char* deleteMod = 0;
		   script = "onDisabled";
		   deleteMod = "selfDelete";

		   mDataBlock->onDisabled_callback( this, "selfDelete" );
	   }
   }
}
...
void Player::getDamageLocation(const Point3F& in_rPos, const char *&out_rpVert, const char *&out_rpQuad)
{
   ...
   //F32 zHeight = boxSize.z * mObjScale.z;
   F32 zTorso  = mDataBlock->boxTorsoPercentage * mObjScale.z;
   F32 zHead   = mDataBlock->boxHeadPercentage * mObjScale.z;

 /*  zTorso *= zHeight;
   zHead  *= zHeight;*/
   //Con::printf("%f   %f   %f", newPoint.x, newPoint.y, newPoint.z );
   ...
   {
	   if (/*newPoint.y >= 0.0f*/true)
	   {
		   if (newPoint.x <= mDataBlock->boxTorsoLeftPercentage )
			   out_rpQuad = "left";
		   else if( newPoint.x >= mDataBlock->boxTorsoRightPercentage )
			   out_rpQuad = "right";
		   else
			   out_rpQuad = "middle";
	   }
   ...
   else if(dStrcmp(out_rpVert, "legs") == 0) 
   {
	   if (newPoint.x <= mDataBlock->boxLegsLeftPercentage )
		   out_rpQuad = "left";
	   else if( newPoint.x >= mDataBlock->boxLegsRightPercentage )
		   out_rpQuad = "right";
	   else
		   out_rpQuad = "middle";
   }
   ...
   {
	   if ( newPoint.x <= mDataBlock->boxHeadLeftPercentage )
	   {
		   //两侧方向
		   /*if (  newPoint.y < mDataBlock->boxSize.y / 2 && newPoint.y > -mDataBlock->boxSize.y / 2 )
		   {
				out_rpQuad = "middle";
		   }
		   else
	   ...
	   else if( newPoint.x >= mDataBlock->boxHeadRightPercentage )
	   {
		   //两侧方向
		  /* if (  newPoint.y < mDataBlock->boxSize.y / 2 && newPoint.y > -mDataBlock->boxSize.y / 2 )
		   {
	  ...
	  // {
		 ...
		 //  case 8: out_rpQuad = "right_front";    break;*/
			//case 0: out_rpQuad = "left";      break;
			//case 1: out_rpQuad = "middle";    break;
			//case 2: out_rpQuad = "right";     break;
			//case 3: out_rpQuad = "left";    break;
			//case 4: out_rpQuad = "middle";  break;
			//case 5: out_rpQuad = "right";   break;
			//case 6: out_rpQuad = "left";     break;
			//case 7: out_rpQuad = "middle";   break;
			//case 8: out_rpQuad = "right";    break;
...
bool Player::isValidDamageLocation( const Point3F& in_rPos, const Point3F& start, const Point3F& end )
{
	const char *buffer1;
	const char *buffer2;
 
	getDamageLocation( in_rPos, buffer1, buffer2 );

	char *buff = Con::getReturnBuffer(128);
	dSprintf(buff, 128, "%s%s", buffer1, buffer2);
	if( strcmp(buff, "headmiddle" ) == 0 && !mBrokenArray[0] )
	{
		return true;
	}
	else if ( !strcmp(buff, "headleft" ) || !strcmp( buff, "headright" ) )
	{
		return true;
		//侧面击中
		//F32 boxWidthMax = mObjBox.maxExtents.x;
		//F32 boxWidthMin = mObjBox.minExtents.x;
		//F32 boxLangthMax = mObjBox.maxExtents.y;
		//F32 boxLangthMin = mObjBox.minExtents.y;
		//mObjBox.maxExtents.x = mDataBlock->boxHeadRightPercentage;
		//mObjBox.minExtents.x = mDataBlock->boxHeadLeftPercentage;
		//mObjBox.maxExtents.y = mDataBlock->boxHeadRightPercentage;
		//mObjBox.minExtents.y = mDataBlock->boxHeadLeftPercentage;
		//resetWorldBox();
		//bool test = mWorldBox.collideLine( start, end );
		//mObjBox.maxExtents.x = boxWidthMax;
		//mObjBox.minExtents.x = boxWidthMin;
		//mObjBox.maxExtents.y = boxLangthMax;
		//mObjBox.minExtents.y = boxLangthMin;
		//resetWorldBox();
		//return test;
	}
	else if( strcmp( buff, "torsomiddle" ) == 0 && !mBrokenArray[1] )
	{
		return true; 
	}
	else if( strcmp( buff, "torsoleft") == 0 && !mBrokenArray[2] )
	{	
		return true;
	}
	else if( strcmp( buff, "torsoright") == 0 && !mBrokenArray[3] )
	{
		return true;
	}
	else if( strcmp( buff, "legsleft") == 0 && !mBrokenArray[4] )
	{
		return true;
	}
	else if( strcmp( buff, "legsright") == 0 && !mBrokenArray[5] )
	{
		return true;
	}
	else
...
void Player::updateMove(const Move* move)
{
   ...
   if ((mState == MoveState || (mState == RecoverState && mDataBlock->recoverRunForceScale > 0.0f)) && mDamageState == Enabled)
   {
	  ...
	  else if ( mDataBlock->isFly && move->x == 0 && move->y == 0 && move->z != 0 )
	  {
		  moveSpeed = mFabs( mDataBlock->maxProneForwardSpeed * move->z );	  
	  }
      else
      {
         if ( mSwimming )
   ...
   // Acceleration due to 
   VectorF acc;
   if( mDataBlock->isFly || mMasterObject != NULL  )
	   acc.zero();
   else
	  acc.set(0.0f, 0.0f, mGravity * mGravityMod * TickSec);
   ...
   //if( mPose == Jump )
	  // move->trigger[sJumpTrigger] = true;
   if (move->trigger[sJumpTrigger] && canJump() /*|| mPose == Jump*/ )// !isMounted() && 
   {
   ...
   if( isJump )  //跳跃的时候只有向下的重力
   {
	   acc.zero();
	   mVelocity.zero();
   }
   else if( mActionType != -1 )        //做位移动作的时候所有的速度为0
   {
	   acc.zero();
	   mVelocity.zero();
   }
...
bool Player::setActionThread(const char* sequence,bool hold,bool wait,bool fsp, bool forceSet )
{
   for (U32 i = 0; i < mDataBlock->actionCount; i++)
   {
	  ...
	  if (i == 0 && !dStricmp(anim.name,sequence))
	  {
		  clearActionInfo();
		  return false;
	  }
      ...
      {
		 clearActionInfo();
		 String yuandi( sequence );
		 if( yuandi.compare("y_",2 , 1 ) == 0 )    //原地动作
		 {
			 mActionType = 0;
			 mActionPos = getTransform();
			 mActionNewPos = getPosition();
		 }
		 if( yuandi.compare("w_",2, 1 ) == 0 )   //动作的前2个字符等于"w_"认为是带位移动作
		 {
			 mActionType = 1;
			 MatrixF mat;
			 //getShapeNode( &mActionPos, "center"/*"Bip01 Footsteps"*/);   //做动作前，获取当前Bip01 Footsteps点，作为基准点。位移信息根据这个点计算得到
			 mActionPos = getTransform();
		 }
		 if ( forceSet )
		 {
...
void Player::setActionThread(U32 action,bool forward,bool hold,bool wait,bool fsp, bool forceSet)
{
   if (!mDataBlock || (mActionAnimation.action == action && mActionAnimation.forward == forward && !forceSet))
      return;
   //Con::printf("mVelocity = %f", mVelocity.len() );
   /*//testPlayer start
   U32 mountObjCount = getMountedObjectCount();
   if ( action != 1 && mountObjCount != 0 )
   {
	   for ( U32 i = 0; i < mountObjCount; i++ )
	   {
			Player *mountPlayer = NULL;
			SceneObject *mountObj = NULL;
			mountObj = getMountNodeObject( i );
			mountPlayer = dynamic_cast< Player *>( mountObj );
			if ( mountPlayer != NULL )
			{
				mountPlayer->setActionThread( action, forward, hold, wait, fsp, forceSet );
				mountPlayer->setMaskBits(ActionMask);
			}
	   }
   }//testPlayer End*/
   ...
   if (anim.sequence != -1)
   {
      ...
      if (sUseAnimationTransitions && (action != PlayerData::LandAnim || !(mDataBlock->landSequenceTime > 0.0f && !mDataBlock->transitionToLand)) && !forceSet)
      {
...
void Player::updateActionThread()
{
   ...
   if(mActionAnimation.action != PlayerData::NullAnimation)
      if (mActionAnimation.forward)
	  {
		 ...
		 if (mActionAnimation.atEnd && mActionAnimation.holdAtEnd && isServerObject() && !bIsMoveState)
		 {
			 const char* testTempActionName = mDataBlock->actionList[mActionAnimation.action].name;
			 if(testTempActionName == NULL)
			 {
				 testTempActionName = "firstFall1";
			 }
			 else
			 {
				 clearActionInfo();
			 ...
			 actionNameTest = testTempActionName;
			 mDataBlock->animationDone_callback( this, actionNameTest );
		 } 
		 if( isServerObject() )
		 {
			 if( mActionType == 0 )
			 {
				 //const char* npcName = mDataBlock->getName();
				 const char* actionName	=	mDataBlock->actionList[mActionAnimation.action].name;
				 mActionName = actionName;
				 S32 count = mShapeInstance->getKeyframeNumber(mActionAnimation.thread);  //获取动作帧数
				 //Con::printf("count = %d", count );
				 int actionCount = mDataBlock->ACTIONNUM;
				for( int i = 0; i < actionCount; i++ )
				{
					if( mDataBlock->actionArray[i] != NULL )
					{
						if( _stricmp(mDataBlock->actionArray[i]->actionName, actionName ) == 0 )
						{
							setWeiyi( mDataBlock->actionArray[i]->action[count], count );
							setbox(mDataBlock->actionArray[i]->actionBox[count]);
							break;
						}
					}
				}
		 ...
		 {
			 //const char* tempActionName = mDataBlock->actionList[mActionAnimation.action].name;
			 //Con::printf("client is %s", tempActionName);
			 if( mActionType == 1 )
			 {
				 Point3F nodePos;
				 MatrixF mat;
				 mat.mul(0);
				 if( getShapeNode( &mat, "center"/*"Bip01 Footsteps"*/) )
				 {
					 const char* npcName = mDataBlock->getName();
					 const char* actionName	=	mDataBlock->actionList[mActionAnimation.action].name;
					 S32 count = mShapeInstance->getKeyframeNumber(mActionAnimation.thread);  //获取动作帧数
					 nodePos = mat.getPosition();
					 nodePos -= mActionPos.getPosition();     //得出各方向的偏移量
					 Point3F box = getBox();
					 Con::printf("current action frame is %d", count );
					 writeActionInfo(npcName, actionName, nodePos, box, count );
				 } 
			 }
		 }
	  }
   ...
   if(mShapeInstance->getTriggerState(1))
   {
	   ...
	   if( isGhost() )
	   {
		   if (_strnicmp(actionName, "run", strlen("run") ) == 0 
			   || _strnicmp(actionName, "down", strlen("down") ) == 0)
		   {
			   triggeredLeft = true;
		   }
	   }

	   mShapeInstance->setTriggerState(1 , false);

	   if(isServerObject() )
	   {
		   if( !dStricmp( actionName, "takeoff" ) )
		   {
			   mDataBlock->jumpInair_callback( this );
		   }
		   mDataBlock->OnAnimationTrigger_callback( this, actionName );
	   }
      ...
      if( triggeredLeft || triggeredRight )
      {
               ...
               STATIC_COLLISION_TYPEMASK | VehicleObjectType, &rInfo ) )
         {
            ...
            if( rInfo.t <= 0.5 && mWaterCoverage == 0.0
				&& material && mDataBlock->footPuffEmitter /*&& material->mShowDust*/ )
            {
       ...
       !mActionAnimation.holdAtEnd && (mActionAnimation.delayTicks -= !mMountPending) <= 0)
   {
      ...
      if ( isServerObject() && mActionAnimation.action >= PlayerData::NumTableActionAnims )
	  {
		  if( !bIsMoveState )
		  {
			  ...
			  String actionName;  
			  actionName = tempActionName;
			  mDataBlock->animationDone_callback( this, actionName );
		  }
	  }
        ...
        (mActionAnimation.action != PlayerData::NullAnimation) )
   {
	  ...
	  if( mActionSpeed.x != 0 )         //改变动作的播放速度
	  {
		  S32 count = mShapeInstance->getKeyframeNumber(mActionAnimation.thread);  //获取动作帧数
		  if( count >= mActionSpeed.y && count <= mActionSpeed.z )
		  {
			  scale = mActionSpeed.x;
		  }
	  }
...
void Player::pickBestMoveAction(U32 startAnim, U32 endAnim, U32 * action, bool * forward) const
{
   ...
   *forward = false;
   ...
   //Con::printf("current action name is ======= %s", mDataBlock->actionList[*action].name );
}
...
void Player::updateAnimation(F32 dt)
{
   ...
   //Con::printf("updateAnimation( %f )", dt);
   ...
   if (mImageStateThread)
	...
	if (mNewActionAnimation.thread)
	{
		mShapeInstance->advanceTime(dt,mNewActionAnimation.thread);
		if( mShapeInstance->getPos(mNewActionAnimation.thread) == 1 )
		{
			clearNewActionThread();
		}
	}
...
Point3F Player::_move( const F32 travelTime, Collision *outCol )
{
   ...
   for (; count < sMoveRetryCount; count++) {
          ...
          mFabs(distance.z) < mObjBox.len_z())
      {
         ...
         if (eaPolyList.isEmpty())
         {
			...
			//Con::printf("totalMotion += (end - start).len()   start = end");
            start = end;
            break;
         }
      }
   ...
   if (count == sMoveRetryCount)
   {
	  ...
	  //Con::printf("start = initialPosition");
      mVelocity.set(0.0f, 0.0f, 0.0f);
   }
...
bool Player::updatePos(const F32 travelTime)
{
   ...
   else
   {
	  ...
	  {
		  if( mActionType == 0 )
			 newPos =  mActionNewPos;//mat.setColumn(3,mActionNewPos);
		  else
			  newPos = delta.posVec;
	  }
      else
...
void Player::setPosition(const Point3F& pos,const Point3F& rot)
{
	...
	{
		if(NULL !=  mMasterObject)
		{
			if ( mMasterObject->isClientObject() )
			{
				mat = mMasterObject->getRenderTransform();
			}
			else
			{
				SceneObject * clientMasterObject = (SceneObject * )mMasterObject->getClientObject();
				if(clientMasterObject)
				{
					mat = clientMasterObject->getRenderTransform();
				}
			}
			...
			{
				mat.mul( image.dataBlock->eyeOffset );
			}
...
/*
	...
	newpos.x += actionMove.x;
	newpos.y += actionMove.y;
	newpos.z += actionMove.z;
...
void Player::getAttack1Vector( VectorF * vec, int whichPlayer)
{
	if ( mMasterObject != NULL )
	{
		if(whichPlayer == 1)
		{
			//于SetImageAim过程中，对象可能销毁，重新获取碰撞对象，向量可不重新计算，保持同武器指向一致
			getContainer()->castRay(gGunOri_0 , gGunDir_0 , (TerrainObjectType | InteriorObjectType | StaticObjectType) | (PlayerObjectType | VehicleObjectType | DamagableItemObjectType | ItemObjectType) , &GunRayinfo_0);
			/*if ( GunRayinfo_0.object != NULL )
			{
				if ( GunRayinfo_0.object->m_DataType == PLAYER_DATA_EXPLODE 
					|| GunRayinfo_0.object->m_DataType == PROJECTILE_DATA_EXPLODE
					|| GunRayinfo_0.object->m_DataType ==STATICSHAPE_DATA_EXPLODE )
				{
					*vec  = gGunV0;
					return;
				}
		...
		{
			//于SetImageAim过程中，对象可能销毁，重新获取碰撞对象，向量可不重新计算，保持同武器指向一致
			getContainer()->castRay(gGunOri_1 , gGunDir_1 , (TerrainObjectType | InteriorObjectType | StaticObjectType) | (PlayerObjectType | VehicleObjectType | DamagableItemObjectType |ItemObjectType) , &GunRayinfo_1);
			/*if ( GunRayinfo_1.object != NULL )
			{
				if ( GunRayinfo_1.object->m_DataType == PLAYER_DATA_EXPLODE 
					|| GunRayinfo_1.object->m_DataType == PROJECTILE_DATA_EXPLODE 
					|| GunRayinfo_1.object->m_DataType == STATICSHAPE_DATA_EXPLODE )
				{
					*vec  = gGunV1;
					return;
				}
	...
	{		
		getContainer()->castRay(gMouserayOri , gMouserayDir , (TerrainObjectType | InteriorObjectType | StaticObjectType) | (PlayerObjectType | VehicleObjectType | DamagableItemObjectType | ItemObjectType) , &MouseRayinfo);
		Point3F pos; 
		getAttackPoint1(&pos);
		/*if ( MouseRayinfo.object != NULL )
		{
			if ( MouseRayinfo.object->m_DataType == PLAYER_DATA_EXPLODE
				|| MouseRayinfo.object->m_DataType == PROJECTILE_DATA_EXPLODE 
				|| MouseRayinfo.object->m_DataType == STATICSHAPE_DATA_EXPLODE )
			{
				(*vec) = MouseRayinfo.point - pos;
				return;
			}
		}	*/	
	}
	/*Point3F sourcePoint;
	sourcePoint.set( 0.0f, 0.0f, 0.0f );
	if ( gPcvr != NULL &&  mMasterObject != NULL )
	{
		if ( whichPlayer == 1 )
		{
			sourcePoint = gPcvrPointS0;
			*vec  = gGunV0;
		}
		else
		{
			sourcePoint = gPcvrPointS1;
			*vec  = gGunV1;
		}
	}
	else
	{
		sourcePoint = gMouseScreenPt;
		Point3F pos; 
		getAttackPoint1(&pos);
		(*vec) = MouseRayinfo.point - pos;
	}

	Point3F souPoint[4], dstPoint[4], nearPoint[4], farPoint[4];
	RayInfo collisionRay[4];
	souPoint[0].set( sourcePoint.x - 35, sourcePoint.y-35, sourcePoint.z );
	souPoint[1].set( sourcePoint.x + 35, sourcePoint.y-35, sourcePoint.z );
	souPoint[2].set( sourcePoint.x + 35, sourcePoint.y+35, sourcePoint.z );
	souPoint[3].set( sourcePoint.x - 35, sourcePoint.y+35, sourcePoint.z );
	for ( int i = 0; i < 4; i++  )
	{
		dstPoint[i].set( 0.0f, 0.0f, 0.0f );
		nearPoint[i].set( 0.0f, 0.0f, 0.0f );
		farPoint[i].set( 0.0f, 0.0f, 0.0f );
		collisionRay[i].object = NULL;
		collisionRay[i].point.set( 0.0f, 0.0f, 0.0f );
	}
	GameTSCtrl *mRootCtrl = NULL;
	if ( Sim::findObject("PlayGui", mRootCtrl ) )
	{
		for ( int index = 0; index < 4; index++ )
		{	
			if ( mRootCtrl != NULL )
			{
				mRootCtrl->upprojectEx( souPoint[index], &dstPoint[index], &nearPoint[index], &farPoint[index] );
				getContainer()->castRay( nearPoint[index], farPoint[index], PlayerObjectType | StaticShapeObjectType | ProjectileObjectType, &collisionRay[index] );
			}
		}
	}
	for ( int id = 0; id < 4; id++ )
	{
		if ( collisionRay[id].object != NULL )
		{
			if ( collisionRay[id].object->m_DataType == PLAYER_DATA_EXPLODE ||
				collisionRay[id].object->m_DataType == STATICSHAPE_DATA_EXPLODE ||
				collisionRay[id].object->m_DataType == PROJECTILE_DATA_EXPLODE )
			{
				Point3F attackPoint1;
				attackPoint1.set( 0.0f, 0.0f, 0.0f );
				getAttackPoint1( &attackPoint1 );
				VectorF newVec;							
				newVec = collisionRay[id].point - attackPoint1;
...
void Player::getAttack2Vector( VectorF * vec, int whichPlayer)
{
	if ( mMasterObject != NULL )
	{
		if(whichPlayer == 1)
	...
	{
		Point3F strPos;
		getAttackPoint( &strPos, 1 );
		//getContainer()->castRay(/*gMouserayOri*/strPos, gMouserayDir, (TerrainObjectType | InteriorObjectType | StaticObjectType) | (AIObjectType | VehicleObjectType | DamagableItemObjectType | ItemObjectType) , &MouseRayinfo);
		getContainer()->castRay(/*gMouserayOri*/strPos, gMouserayDir, (TerrainObjectType | InteriorObjectType | StaticObjectType) | AIObjectType, &MouseRayinfo);
		Point3F pos; 
		getAttackPoint1(&pos);
		(*vec) = MouseRayinfo.point - pos;
	}
...
U32 Player::packUpdate(NetConnection *con, U32 mask, BitStream *stream)
{
   ...
   stream->writeAffineTransform(mActionPos);
   stream->write(mActionType);
   stream->write(mRunName);
   mathWrite(*stream, mActionNewPos);
   mathWrite(*stream, mActionSpeed);
   stream->writeFlag( isJump );
   ...
   if( stream->writeFlag( true ) )
   {
	   for ( int i = 0; i < NumMountBox; i++ )
	   {
		   S32 value = mountBox[i];
		   stream->write( value );
	   }
	   stream->writeFlag(bIsMountBox);
   }

   if( stream->writeFlag( true ) )
   {
	   stream->writeFlag(bStopCurAction);
	   stream->writeFlag(bReversePlay);
   }
...
void Player::unpackUpdate(NetConnection *con, BitStream *stream)
{
   ...
   stream->readAffineTransform( &mActionPos);
   stream->read( &mActionType );
   stream->read( &mRunName );
   mathRead( *stream, &mActionNewPos );
   mathRead( *stream, &mActionSpeed );
   isJump = stream->readFlag();
   if (stream->readFlag())
   ...
   if( stream->readFlag() )
   {
	   for ( int i = 0; i < NumMountBox; i++ )
	   {
		   stream->read( &mountBox[i] );
	   }
	   bIsMountBox = stream->readFlag();
   }
	
	if (stream->readFlag()) 
	{
		bStopCurAction = stream->readFlag();
		bReversePlay = stream->readFlag();
	}
...
{
	bool forceSet = true;
	object->setIsMoveState(false);
	return object->setActionThread( name, hold, true, fsp, forceSet );
}
...
DefineEngineMethod( Player, setRunName, void, ( const char* name),,
				   "set Run action" )
{
	object->setRunName( name );
}

DefineEngineMethod( Player, setTurn, void, ( F32 turnAngle, const char* actionName ),(0,NULL),
				   "let player trun by z" )
{
	object->initTurnAngle(turnAngle, actionName);
}

DefineEngineMethod( Player, setJump, void, ( SceneObject *obj, F32 hight, F32 weiyiGravity ),( NULL, 1,-50.0 ),
				   "let player jump. Apoint a end point and fly time" )
{
	if( obj ==NULL )
	{
		Con::errorf("please write a place");
		return;
	}
	if( hight <= 0 )
	{
		Con::errorf("hight must > 0");
		return;
	}
	Point3F end = obj->getPosition();
	//object->setActionThread( "takeoff", false, true, true, true );
	object->initJump(end, hight, weiyiGravity);
}

DefineEngineMethod( Player, setFly, void, ( bool  value),(true),
				   "let player trun by z" )
{
	object->setFly( value );
}

DefineEngineMethod( Player, setActionSpeed, void, ( F32  value, U32 start, U32 end ),(1, 0, 1000),
				   "setActionSpeed" )
{
	if( start < 0 || start >= end )
		return;
	object->setActionSpeed( value, start, end );
}

DefineEngineMethod( Player, getThreadIndex, S32, ( const char* name ),," ")
{
	return object->getThreadIndex( name );
}

ConsoleMethod( Player, getAttackPoint1, const char*,2, 2,"attackPoint1")
{
	Point3F p;
	object->getAttackPoint1(&p);
	char* buff = Con::getReturnBuffer(100);
	dSprintf(buff,100,"%g %g %g",p.x,p.y,p.z);
	return buff;
}

ConsoleMethod( Player, getAttackPoint2, const char*,2, 2,"attackPoint2")
{
	Point3F p;
	object->getAttackPoint2(&p);
	char* buff = Con::getReturnBuffer(100);
	dSprintf(buff,100,"%g %g %g",p.x,p.y,p.z);
	return buff;
}

ConsoleMethod( Player, getAttackPoint3, const char*,2, 2,"attackPoint1")
{

	Point3F p;
	object->getAttackPoint3(&p);
	char* buff = Con::getReturnBuffer(100);
	dSprintf(buff,100,"%g %g %g",p.x,p.y,p.z);
	return buff;
}

ConsoleMethod( Player, getAttackPoint4, const char*,2, 2,"attackPoint2")
{
	Point3F p;
	object->getAttackPoint4(&p);
	char* buff = Con::getReturnBuffer(100);
	dSprintf(buff,100,"%g %g %g",p.x,p.y,p.z);
	return buff;
}

ConsoleMethod( Player, getAttackPoint5, const char*,2, 2,"attackPoint2")
{
	Point3F p;
	object->getAttackPoint5(&p);
	char* buff = Con::getReturnBuffer(100);
	dSprintf(buff,100,"%g %g %g",p.x,p.y,p.z);
	return buff;
}
...
ConsoleMethod( Player, getAttack2Vector,const char*,3, 3, "attack2 vector")
{
	VectorF v;
	U32 index = dAtoi(argv[2]);
	object->getAttack2Vector(&v, index);
	char* buff = Con::getReturnBuffer(100);
	dSprintf(buff,100,"%g %g %g",v.x,v.y,v.z);
	return buff;
}

ConsoleMethod( Player , mountBoxToNode,  void, 4, 4, "mount box to node by index")
{
	int index = dAtoi( argv[3] );
	if( index < 0 || index >25 )
	{
		Con::printf("out of range,please check the index!");
		return;
	}
	StaticShape *box;
	if( Sim::findObject( argv[2], box ) )
	{
		object->mountBoxToNode( box->getId(), index );
	}
	else
	{
		Con::errorf("Player::mountBoxToNode is not find the box!");
	}
	return;
}
...
void Player::prepRenderImage( SceneRenderState* state )
{
   ...
   */
   ...
   // and mounted shapes.
   ShapeBase::_prepRenderImage( state, 
									   renderPlayer, 
									   renderItems );

   return renderMountBox();
}

void Player::renderMountBox()
{
	if ( !bIsMountBox )
	{
		return;
	...
	bool isMountBox = false;
	for( int i = 0; i < NumMountBox; i++ )
	{
		if ( mountBox[i] != -1 )
		{
			StaticShape *box;
			if( Sim::findObject( mountBox[i], box ) )
			{
				if( box->isDestroyed() )
				{
					mountBox[i] = -1;
				}
				else if( mDataBlock->mountBoxNode[i] != -1 )
				{
					MatrixF mat;
					isMountBox = true;

					getBoxMountMatrixF( mat, i );
					box->setTransform(mat);
					box->setRenderTransform(mat);
					if ( box->isServerObject() )
					{
						StaticShape* boxClient = NULL;
						boxClient = (StaticShape*) ( box->getClientObject() );
						if ( boxClient )
						{
							boxClient->setTransform(mat);
							boxClient->setRenderTransform(mat);
						}
					} 
					else
					{
						StaticShape* boxServer = NULL;
						boxServer = (StaticShape*) ( box->getServerObject() );
						if ( boxServer )
						{
							boxServer->setTransform(mat);
							boxServer->setRenderTransform(mat);
						}
					}
				}
			}
...
void Player::getAttackPoint1( Point3F *pos )
{//getAttackPoint1Pos(pos);
	if ( mMasterObject == NULL )
	{
		getAttackPoint1Pos(pos);
	}
...
void Player::getAttackPoint2( Point3F *pos )
{
	getAttackPoint2Pos(pos);
}

void Player::getAttackPoint3( Point3F *pos )
{
	getAttackPoint3Pos(pos);
}

void Player::getAttackPoint4( Point3F *pos )
{
	getAttackPoint4Pos(pos);
}

void Player::getAttackPoint5( Point3F *pos )
{
	getAttackPoint5Pos(pos);
}

void Player::setRunName( const char* runName )
{
	mRunName = 0;
	if( runName != NULL )
	{
		...
		{
			const PlayerData::ActionAnimation &anim = mDataBlock->actionList[i];
			if( _stricmp(anim.name, runName) == 0 )
			{
				mRunName = i;
				break;
			}
...
void Player::initTurnAngle( F32 turnAngle, const char* actionName )
{
	mTurnAngle = ( turnAngle / 180 ) * M_PI_F;
	if( actionName != NULL )
	{
		setActionThread( actionName, false, true, true );
		setTurnSpeed( turnAngle, actionName );
	}
	setPose(Turn);
}

void Player::setTurnSpeed( F32 turnAngle, const char* actionName )
{
	for( U32 i = 0; i < mDataBlock->actionCount; i++ )
	{
		const PlayerData::ActionAnimation anim = mDataBlock->actionList[i];
		if (!_stricmp(anim.name,actionName)) 
		{
			F32 time = mShapeInstance->getDuration(mActionAnimation.thread);
			F32 count = time / TickSec;
			mAngleSpeed = mFabs(turnAngle) / count;
			return;
		}
	}
}
...
void Player::setTurn()
{
	F32 angleSpeed = ( mAngleSpeed / 180 ) * M_PI_F;
	F32 turnAngle = 0;
    if( mTurnAngle >= 0 )
	{
		mTurnAngle -= angleSpeed;
	...
	if( mFabs(mTurnAngle) < angleSpeed )
	{
		//当需要转的度数小于0.001时，认为转到了
		mTurnAngle = 0;
		setPose( StandPose );
		return;
	}
	MatrixF rotMat;
	rotMat.set( EulerF(0.0, 0.0, turnAngle) );
	MatrixF curMat = getTransform();
	curMat.mul( rotMat );
	setTransform( curMat );
}

void Player::setWeiyi( VectorF vec , U32 count)
{
	MatrixF mat;
	mat = getTransform();
	Point3F newPos;
	VectorF forward = mat.getForwardVector();
	forward *= vec.y;
	VectorF right = mat.getRightVector();
	right *= vec.x;
	VectorF newVec = right+forward;
	newPos = mActionPos.getPosition() + newVec;
	if( dStricmp(mActionName, "y_climb") != 0 )      //做攀爬动作的时候不计算Z方向的高度
		newPos.z += vec.z;

	if( dStricmp(mActionName, "y_climb") != 0 && dStricmp(mActionName, "y_run3") != 0 )
	{
		F32 S = ( mGravity * (count*TickSec)*(count*TickSec) ) / 2;
		newPos.z += S;
	}
	RayInfo ri;
    bool hit = false;
	Point3F curPos = mat.getPosition();
    Point3F startPoint(curPos.x, curPos.y, curPos.z+1);
	Point3F endPoint(curPos.x, curPos.y, curPos.z-10);
	hit = getContainer()->castRay(startPoint, endPoint, sCollisionWeiyiMask, &ri);   
	if( hit )
	{
		if( newPos.z < ri.point.z )
		{
			newPos.z = ri.point.z;
		}
	}
    mActionNewPos = newPos;
}

Point3F Player::getBox()
{
	MatrixF zuoxia;
	if( getShapeNode( &zuoxia, "zuoxia") )
	{
		MatrixF youshang;
		if( getShapeNode( &youshang, "youshang") )
		{
			Point3F youshangPos = youshang.getPosition();
			Point3F zuoxiaPos = zuoxia.getPosition();
			//Point3F pos = youshangPos - zuoxiaPos;
			F32 box_z =  mFabs(youshangPos.z - zuoxiaPos.z) ;
			F32 box_y = mFabs(youshangPos.y - zuoxiaPos.y);
			F32 box_x = mFabs(youshangPos.x - zuoxiaPos.x);
			Point3F pos(box_x, box_y, box_z);
			return pos;
		}
	}
		return mDataBlock->boxSize;
}

void Player::setbox( Point3F box )
{
	//mDataBlock->boxSize = box;
	Point3F boxSize(box );
	//Con::printf("count is  %d      box size is = %f   %f   %f", count, box.x, box.y, box.z);
	// Object and World Boxes...
	mObjBox.maxExtents.x = boxSize.x * 0.5f;
	mObjBox.maxExtents.y = boxSize.y * 0.5f;
	mObjBox.maxExtents.z = boxSize.z;
...
void Player::initJump( Point3F endPos, F32 high, F32 weiyiGravity )
{
	mWeiyiGravity = weiyiGravity;
	Point3F startPos = getPosition();
	F32 highestPos = (startPos.z > endPos.z) ? startPos.z : endPos.z;
	F32 len = highestPos + high;
	//Z轴上的初速度
	F32 gravity = -mWeiyiGravity;
	F32 speed_z = mSqrt(2 * (len-startPos.z) * gravity);
	F32 time1 = speed_z / gravity;				   //前半段上升的时间
	F32 distance = highestPos +high - endPos.z;   //后半段下落的距离
	F32 time2 = mSqrt(2*distance / gravity);
	F32 time = time1 + time2;
	VectorF velocity;
	mJumpVelocity.x = (endPos.x - startPos.x)/time;
	mJumpVelocity.y = (endPos.y - startPos.y)/time;
	mJumpVelocity.z = speed_z;
	isJump = true;
}
...
{
	//mWeiyiGravity = weiyiGravity;
	Point3F newPosition, oldPosition;
	oldPosition = getPosition();
	mJumpVelocity.z += mWeiyiGravity * TickSec;
	newPosition = oldPosition + mJumpVelocity * TickSec;
	RayInfo ri;
	bool hit = false;
	hit = getContainer()->castRay(oldPosition, newPosition, sCollisionWeiyiMask, &ri);   
	if( hit )
	{
		if( newPosition.z <= ri.point.z )
		{
			newPosition.z = ri.point.z + 0.01;
			mJumpVelocity.zero();
			isJump = false;
			mDataBlock->jumpDown_callback( this );
		}
	}
	MatrixF mat = getTransform();
...
void Player::writeActionInfo( const char* name, const char* actionName, Point3F p, Point3F box, U32 count )
{
	mDataBlock->onwriteActionInfo_callback(this, name, actionName, p, box, count);
}

S32 Player::getThreadIndex(const char* sequenceName)
{
	S32 a = mShapeInstance->getShape()->findSequence( sequenceName );
	return a;
}

void Player::setBrokenMsg( U32 locationId, bool isBroken )
{
	if( locationId < 0 || locationId > 6 )
	{
		Con::errorf("setBrokenMsg id out of range! %d", locationId );
		return;
	}
	else
		mBrokenArray[ locationId ] = isBroken;
}

bool Player::getBrokenMsg( U32 locationId )
{
	if( locationId < 0 || locationId > 6 )
	{
		Con::errorf("setBrokenMsg id out of range! %d", locationId );
		...
		return mBrokenArray[ locationId ];
}

void Player::setFly( bool value )
{
	mDataBlock->isFly = value;
}

void Player::setActionSpeed( F32 value, F32 start, F32 end )
{
	mActionSpeed.x = value;
	mActionSpeed.y = start;
	mActionSpeed.z = end;
}

void Player::clearActionInfo()
{
	mRunName = 0;
	mActionType = -1;
	mActionPos.mul(0);
	...
	mActionName = "";
	mActionSpeed.zero();
	setbox(mDataBlock->boxSize);
}

const char* Player::getCurActionName()
{
	return mDataBlock->actionList[mActionAnimation.action].name;
}

ConsoleMethod( Player, getCurActionName, const char*, 2, 2, "" )
{
	return object->getCurActionName();
}

// set direction
void Player::SetImageAim(MatrixF& mat)
{
	Point3F	pos;
	mat.getColumn(3, &pos);
	Point3F   testPoint;
	Point3F   camForwardVec;
	bool hit = false;
	F32 distance1 = 0;
	F32 distance2 = 0;
	Point3F masterObjectPos;
	...
	if (!gPcvr)
	{
		if ( gPhysicsPlugin )
		{
			...
			if ( world )
			{
				//world->castRay(gMouserayOri , gMouserayDir , &MouseRayinfo , Point3F(1 , 1 , 1));
			}
		}
		else 
		{
			U32 colMask =	TerrainObjectType | 
										InteriorObjectType | 
										StaticShapeObjectType | 
										ItemObjectType | 
										WaterObjectType | 
			...
			if(mMasterObject != NULL)
			{
				//mMasterObject->getContainer()->castRay(gMouserayOri, gMouserayDir, (TerrainObjectType | InteriorObjectType | StaticObjectType) | ( AIObjectType | VehicleObjectType | DamagableItemObjectType | ItemObjectType) , &MouseRayinfo);
				//masterMat = mMasterObject->getTransform();

				if ( mMasterObject->isClientObject() )
				{
					masterMat = mMasterObject->getRenderTransform();
				}
				else
				{
					SceneObject * clientMasterObject = (SceneObject * )mMasterObject->getClientObject();
					if(clientMasterObject)
					{
						masterMat = clientMasterObject->getRenderTransform();
					}
				}
				...
				if (mGunTypeIndex == 2)
				{
					farPlaneVec = gMouseWorldPt1 - masterObjectPos;//far vector 
				}
				else
				{
					farPlaneVec = gMouseWorldPt - masterObjectPos;//far vector 
				}
				
				testPoint = farPlaneVec * 50;
				farPlaneVec *= 1000;
				farPlaneVec = farPlaneVec + masterObjectPos;
				testPoint = testPoint + masterObjectPos;
				hit = getContainer()->castRay(masterObjectPos, farPlaneVec, colMask, &MouseRayinfo);

				if (mSlaveIndex == 0)
				{
					VectorF tempV = masterObjectPos - MouseRayinfo.point;
					distance1 = tempV.len();
				}
				...
				if(mGunTypeIndex == 2)
				{
					Con::executef("onDetectHitNPC", Con::getIntArg(mSlaveIndex));
				}

				camForwardVec = masterMat.getForwardVector();
			}
			else
			{
				//getContainer()->castRay(gMouserayOri, gMouserayDir, (TerrainObjectType | InteriorObjectType | StaticShapeObjectType | ItemObjectType | WaterObjectType |	AIObjectType | FlyShapeObjectType |	VehicleObjectType | ProjectileObjectType) , &MouseRayinfo);
				getContainer()->castRay(gMouserayDir, gMouserayOri, colMask, &MouseRayinfo);
			}

			//getContainer()->castRay(gGunOri_0, gGunDir_0, (TerrainObjectType | InteriorObjectType | StaticObjectType) | (AIObjectType | VehicleObjectType | DamagableItemObjectType | ItemObjectType) , &GunRayinfo_0);
			//getContainer()->castRay(gGunOri_1, gGunDir_1, (TerrainObjectType | InteriorObjectType | StaticObjectType) | (AIObjectType | VehicleObjectType | DamagableItemObjectType |ItemObjectType) , &GunRayinfo_1);
		}
		...
		if (mGunTypeIndex == 10)
		{
			...
			if (mSlaveIndex == 0)
			{
				...
				Point3F addVec(-0.217745f, -0.0239961f, 0.00181482f);
				vec = camForwardVec + addVec;
			}
			else if (mSlaveIndex == 1)
			{
				camForwardVec.normalize();
				Point3F addVec(0.0474285f, 0.119429f, 0.114954f);
				vec = camForwardVec + addVec;
			}
		}
		else
		...
		switch(mSlaveIndex)
		{
			case 0:
				if (distance1 < 3)
				{
					gGunV0 = MouseRayinfo.point - masterObjectPos;
					gGunP0 = masterObjectPos;
					gGunV0.normalizeSafe();
				}
				else
				{
					gGunV0 = MouseRayinfo.point - pos;
					mat.getColumn(3, &gGunP0);
					gGunV0.normalizeSafe();
				}
				break;
			case 1:
				if (distance2 < 3)
				{
					gGunV1 = MouseRayinfo.point - masterObjectPos;
					gGunP1 = masterObjectPos;
					gGunV1.normalizeSafe();
				}
				else
				{
					gGunV1 = MouseRayinfo.point - pos;
					mat.getColumn(3, &gGunP1);
					gGunV1.normalizeSafe();
				}
				break;
			default:
		...
		U32 shapeIndex = getImageShapeIndex(image);
		S32 node = imageData->ejectNode[getImageShapeIndex(image)];
		...
		if (mSlaveIndex == 0)
		{
			mEjectTrans1.mul( mat, nmat );
		}
		else if (mSlaveIndex == 1)
		{
			mEjectTrans2.mul( mat, nmat );
		}
	}
	else
	{
		...
		case 0:
			if (distance1 < 3)
			{
				gGunV0 = GunRayinfo_0.point- masterObjectPos;
				gGunP0 = masterObjectPos;
				gGunV0.normalizeSafe();
			}
			else
			{
				gGunV0 = GunRayinfo_0.point - pos;
				mat.getColumn(3, &gGunP0);
				gGunV0.normalizeSafe();
			}
			break;
		case 1:
			if (distance2 < 3)
			{
				gGunV1 = GunRayinfo_1.point - masterObjectPos;
				gGunP1 = masterObjectPos;
				gGunV1.normalizeSafe();
			}
			else
			{
				gGunV1 = GunRayinfo_1.point - pos;
				mat.getColumn(3, &gGunP1);
				gGunV1.normalizeSafe();
			}
			break;
	...
	camForwardVec.normalize();
	vec -= camForwardVec;
	if (mGunTypeIndex == 10 && mSlaveIndex == 1)
	Con::printf("vec:  %g   %g   %g", vec.x, vec.y, vec.z );*/

}

//通过id索引绑盒点
void Player::getBoxMountMatrixF( MatrixF &matrixf, int index )
{
	S32 point = mDataBlock->mountBoxNode[index];
	if( point != -1 )
	{
		matrixf.mul( getRenderTransform(), mShapeInstance->mNodeTransforms[point] );
	}
	else
	{
		matrixf = getRenderTransform();
	}
}

//通过id绑盒子
void Player::mountBoxToNode( S32 mountedBoxID, int index )
{
	SceneObject *target;
	if (Sim::findObject( mountedBoxID,target))
	{
		bIsMountBox = true;
		mountBox[ index ] = mountedBoxID;
		GameBase * box = dynamic_cast<GameBase*>( target );
		if ( box )
		{
			box->processAfter( this );
		}
	}
	else
	{
		Con::warnf("Player::mountBoxToNode -> mountedBoxID is not an object! mountedBoxID = %d", mountedBoxID);
	}
}

void Player::setIsMoveState(bool isMoveState)
{
	bIsMoveState = isMoveState;
}

DefineConsoleFunction( getTwoVectorAngle, F32, ( const char * obj1ID, const char * obj2ID ),,
					  "Calculate the angle of two vector.\n" )
{
	...
	if ( Sim::findObject( obj1ID, obj1 ) )
	{
		forWardVec1 = obj1->getTransform().getForwardVector();
	}
	else
	{
		return -1;
	...
	if ( Sim::findObject( obj2ID, obj2 ) )
	{
		forWardVec2 = obj2->getTransform().getForwardVector();
	}
	else
	{
		return -1;
	...
	float angle = acosf( mDot( forWardVec1, forWardVec2 ) / ( forWardVec1.len() * forWardVec2.len() ) );

	return angle;
}

ConsoleMethod( Player , unmountBoxToNode,  void, 3, 3, "mount box to node by index")
{
	int index = dAtoi( argv[2] );
	if( index < 0 || index >25 )
	{
		Con::printf("out of range,please check the index!");
		return;
	}
	object->unmountBoxToNode( index );
	return;
}

//通过id解绑盒子
void Player::unmountBoxToNode( int index )
{
	mountBox[ index ] = -1;
}


void Player::newActionThread(const char* seqName)
{
	if ( isGhost() )
	{
		S32 seqIndex = mShapeInstance->getShape()->findSequence(seqName);
		if (seqIndex != -1)
		{
			if (!mNewActionAnimation.thread)
			{
				mNewActionAnimation.name = seqName;
				mNewActionAnimation.action = seqIndex;
				mNewActionAnimation.thread = mShapeInstance->addThread();
			}
			mShapeInstance->setSequence( mNewActionAnimation.thread, mNewActionAnimation.action, 0 );
		}
	}	
}

void Player::clearNewActionThread()
{
	if ( isGhost() )
	{
		if (mNewActionAnimation.thread)
...
DefineEngineMethod( Player, newActionThread, void, ( const char* name ), ,
				   "@brief Set the main action sequence to play for this player.\n\n")
{
	Player *clientObject = NULL;
	clientObject = dynamic_cast< Player* >( object->getClientObject() );
	if ( clientObject != NULL )
	{
		clientObject->newActionThread(name);
	}
}

ConsoleMethod( Player , stopCurAction,  void, 3, 3, "stop current action")
{
	bool stopAction = dAtoi( argv[2] );
	object->stopCurAction( stopAction );
	return;
}

void Player::stopCurAction(bool stopAction)
{
	bStopCurAction = stopAction;
}

ConsoleMethod( Player , setReversePlay,  void, 3, 3, "set reverse play")
{
	bool reversePlay = dAtoi( argv[2] );
	object->setReversePlay( reversePlay );
	return;
}

void Player::setReversePlay(bool reversePlay)
{
	bReversePlay = reversePlay;
}
...
void Player::setLifeTime( U32 lifeTime )
{
	mLifeTime = lifeTime;
}

ConsoleMethod( Player, setLifeTime, void, 3, 3, "" )
{
	U32 lifeTime = dAtoi( argv[2] );
	if( lifeTime <= 0 )
	{
		Con::errorf( "set lift Time is error! %d", lifeTime );
		return;
	}
